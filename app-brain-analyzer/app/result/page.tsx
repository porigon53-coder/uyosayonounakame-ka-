  ...
  const renderPatternByStyle = (styleId: number, forceBigH = false) => {
      ...
      return (
        <>
          {circlePositions.map((pos, idx) => (
            <span
              key={idx}
              className={`absolute ${activeWords.wordA.color} text-sm font-bold`}
              style={{ top: pos.top, left: pos.left }}
            >
              {activeWords.wordA.text}
            </span>
          ))}
          <span
            className={`absolute ${activeWords.wordB.color} text-3xl font-black transform -translate-x-1/2 -translate-y-1/2`}
            style={{ top: "40%", left: "51%" }}
          >
            {activeWords.wordB.text}
          </span>
        </>
      );
    }
    ...
    if (styleId === 1) {
      return circlePositions.map((pos, idx) => (
        <span
          key={idx}
          className={`absolute ${idx % 2 === 0 ? activeWords.wordA.color : activeWords.wordB.color} text-sm font-bold`}
          style={{ top: pos.top, left: pos.left }}
        >
          {idx % 2 === 0 ? activeWords.wordA.text : activeWords.wordB.text}
        </span>
      ));
    }
    ...
    if (styleId === 2) {
      ...
      return (
        <>
          ...
          <span
            className={`absolute ${activeWords.wordB.color} text-4xl font-black`}
      ...
      return (
        <>
          ...
          {evenRightPositions.map((pos, idx) => (
            <span
              key={`r-${idx}`}
              className={`absolute ${activeWords.wordB.color} text-sm font-bold`}
              style={{ top: pos.top, left: pos.left }}
            >
              {activeWords.wordB.text}
            </span>
    ...
    <main className="min-h-screen bg-slate-50 text-slate-800 p-6 flex flex-col items-center">
      <header className="max-w-5xl w-full text-center mb-4">
        ...
        <p className="text-xs md:text-sm text-slate-500 font-medium">
          Web閲覧履歴 ＆ AI検索履歴からあなたの脳内・政治思想スタンスを可視化
        </p>
      </header>
      ...
      {/* Google広告の下に配置するファイル＆AI履歴アップロードスペース */}
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 text-left">
          <div className="flex border-b border-slate-100 mb-4 gap-2">
            <button
              ...
              className={`flex-1 py-2 text-xs md:text-sm font-bold border-b-2 transition ${
                activeTab === "ai"
          ...
          {activeTab === "ai" && (
            <div className="space-y-3">
              <div className="bg-indigo-50/70 border border-indigo-100 rounded-xl p-2.5 text-xs text-indigo-900 leading-relaxed">
                <p className="font-bold mb-0.5">
                  💡 AI履歴を貼り付けて精度をアップ！
                </p>
                ChatGPTやGemini等に下のプロンプトを送信し、返ってきたテキストを貼り付けるとより精密に脳内が可視化されます。
              </div>

              <div className="relative bg-slate-900 rounded-xl p-3 text-slate-100">
                <pre className="text-[10px] md:text-[11px] font-mono whitespace-pre-wrap leading-relaxed opacity-90">
                  {promptText}
                </pre>
                <button
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white font-bold py-2.5 rounded-xl transition text-xs md:text-sm shadow-sm active:scale-95 cursor-pointer"
                >
                  新しいデータで再判定する
                </button>
              </div>
          ...
          {activeTab === "file" && (
            <div className="space-y-3 text-center py-1">
              <p className="text-xs text-slate-600 leading-relaxed text-left">
                ブラウザ（Chrome, Edge,
                Safari等）から出力した閲覧履歴データ（JSON /
                CSV）を直接読み込んで判定します。
              </p>

              <label className="cursor-pointer inline-flex items-center justify-center w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 rounded-xl transition shadow-sm text-xs md:text-sm active:scale-95">
                <span>閲覧履歴ファイルを選択して再判定</span>
                <input
                  type="file"
                  accept=".json, .csv"
