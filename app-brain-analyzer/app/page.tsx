...
export default function HomePage() {
  ...
  const [modalStep, setModalStep] = useState<'intro' | 'questions'>('intro');
...
[
  ...
  const navigateToResultWithSurvey = (surveyAnswers: Record<number, 'YES' | 'NO'>) => {
    const dummyData = questions.map((q) => ({
      title: `${q.category}: ${q.text} -> ${surveyAnswers[q.id] || '未回答'}`,
      domain: surveyAnswers[q.id] === 'YES' ? 'news-liberal' : 'news-conservative',
    }));
      ...
      try {
        ...
        if (file.name.endsWith('.json')) {
          ...
          parsedData = lines.slice(1, 31).map((line) => {
            ...
            return { title: cols[0] || '履歴データ', domain: cols[1] || 'uploaded-history' };
          });
        }
        ...
        alert('ファイルの読み込みに失敗しました。正しいJSONまたはCSV形式か確認してください。');
      }
    };
  ...
  const handleAiTextSubmit = () => {
    ...
    try {
      ...
      alert('テキスト解析に失敗しました。正しい形式で入力されているか確認してください。');
    }
  };
    ...
    <main className="min-h-screen bg-slate-50 text-slate-800 p-6 relative flex flex-col items-center justify-center">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/65 backdrop-blur-md flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-slate-100 w-full max-w-md rounded-2xl p-6 shadow-2xl flex flex-col items-center relative overflow-hidden">
            
            {/* 1. アプリ説明・目的のウェルカム画面 (Step: intro) */}
            {modalStep === 'intro' && (
              <div className="text-center space-y-4 py-2">
                <div className="inline-flex items-center justify-center w-14 h-14 bg-indigo-50 text-indigo-600 rounded-full text-3xl mb-1 shadow-inner">
                  🧠
                </div>
                ...
                <p className="text-xs text-slate-600 leading-relaxed bg-slate-50 p-3.5 rounded-xl border border-slate-100 text-left">
                  本アプリは、<strong>「あなたの関心領域」や「政治・価値観スタンス」</strong>を多角的に解析し、脳内イメージとマップ上に可視化するエンタメ診断ツールです。
                </p>

                <div className="bg-indigo-50/60 border border-indigo-100 rounded-xl p-3 text-left space-y-1.5">
                  <div className="flex items-start gap-2 text-xs text-indigo-950 font-medium">
                    <span className="text-indigo-600 font-bold">🔒 完全プライベート</span>
                    <span>入力されたデータは送信・保存されません。</span>
                  </div>
                  <div className="flex items-start gap-2 text-xs text-indigo-950 font-medium">
                    <span className="text-indigo-600 font-bold">⚡ たった15秒で診断</span>
                    <span>5問の質問に答えるだけで即座に判定可能！</span>
                  </div>
                </div>
                ...
                <div className="pt-2 space-y-3">
                  <button
                    ...
                    className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-sm rounded-xl transition shadow-md active:scale-95 cursor-pointer flex items-center justify-center gap-1.5"
                  >
                    <span>15秒で診断をはじめる（全5問）</span>
                    <span>→</span>
                  </button>
            ...
            {/* 2. 5問アンケート画面 (Step: questions) */}
            {modalStep === 'questions' && questions.length > 0 && (
          ...
          {activeTab === 'ai' && (
            <div className="space-y-4">
              <div className="bg-indigo-50/70 border border-indigo-100 rounded-xl p-3 text-xs text-indigo-900 leading-relaxed">
                ...
                普段お使いのAI（ChatGPT / Claude / Gemini等）に下のプロンプトを送信し、返ってきたテキストをそのまま下に貼り付けてください。
              </div>
          ...
          {activeTab === 'file' && (
            <div className="space-y-4 text-center py-2">
              <p className="text-xs text-slate-600 leading-relaxed text-left">
                ブラウザ（Chrome, Edge, Safari等）の拡張機能などで出力した閲覧履歴データ（JSON / CSV）を直接読み込んで解析します。
              </p>
              ...
              <p className="text-[11px] text-slate-400">※アップロードしたデータはブラウザ内のみで解析され、サーバーへ送信されることはありません。</p>
            </div>
          )}
        </div>
